...
class Main(tk.Frame):  # Класс главного окна
    ...
    def init_main(self):  # Метод для хранения и инициалиации объектов главного окна
        toolbar = tk.Frame(bg="#d7d8e0", bd=2)
        toolbar.pack(side=tk.TOP, fill=tk.X)
        self.add_img = tk.PhotoImage(file="./img/add.png")   # Кнопка
        btn_open_dialog = tk.Button(
            toolbar, bg="#d7d8e0", bd=0, image=self.add_img, command=self.open_dialog
        )
        btn_open_dialog.pack(side=tk.LEFT)

        self.tree = ttk.Treeview(    # Создание таблицы
    ...
    def open_dialog(self):  # Метод для вызова дочернего окна
        Child()

    def records(self, name, tel, email):  # Метод который вызывает метод записи в БД
        self.db.insert_data(name, tel, email)
        self.view_records()

    def view_records(self):  # Метод для вывода данных в виджет таблице
...
class Child(tk.Toplevel):   # Дочернее окно
    ...
    def init_child(self):   # Метод для хранения и инициалиации объектов дочернего окна
        self.title("Добавить")
        ...
        label_name = tk.Label(self, text="ФИО:")  # Создание формы открывающейся в дочернем окне
        ...
        self.entry_name = ttk.Entry(self)  # Создание строк ввода
        ...
        self.btn_cancel = ttk.Button(self, text="Закрыть", command=self.destroy)  # Кнопка для закрытия дочернего окна
        self.btn_cancel.place(x=220, y=170)

        self.btn_ok = ttk.Button(self, text="Добавить")  # Кнопка для добавления дочернего окна
        self.btn_ok.place(x=300, y=170)

        self.btn_ok.bind(
            "<Button-1>",
            lambda event: self.view.records(  # Функционал добавлении информации из полей в БД
...
class DB: # База данных
    ...
    def insert_data(self, name, tel, email):  # Метод на добавление данных в БД
        self.cursor.execute(
